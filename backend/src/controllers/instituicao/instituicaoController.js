...
const post = async (request, response) => {
  try {
    ...
    if (!instituicao.tx_sigla || !instituicao.tx_descricao) {
      return await response.status(400).json({ mensagem: "Campo tx_descrição e tx_sigla é obrigatorio" });
    }
...
const put = async (request, response) => {
  try {
    ...
    if (!instituicao.id_instituicao || !instituicao.tx_descricao || !instituicao.tx_sigla)
      return await response
        .status(400)
        .json({ mensagem: "É necessário o campo id_instituicao, tx_descricao e tx_sigla" });

    const instituicaoExiste = await InstituicaoModel.findByPk(instituicao.id_instituicao);

    if (!instituicaoExiste) return await response.status(404).json({ mensagem: "Instituicao não existe" });
...
const destroy = async (request, response) => {
  try {
    ...
    if (!id) return await response.status(400).json({ mensagem: "É necessário o id da instituicao" });
